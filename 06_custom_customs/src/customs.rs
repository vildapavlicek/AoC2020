...
impl Answers {
    pub fn load() -> Result<Self, Box<dyn std::error::Error>> {
        Ok(Answers(std::fs::read_to_string("./06_custom_customs/input.txt")?))
    }
    ...
    pub fn count_answers(&self) {
        ...
        for group_answers in answers.into_iter() {
            ...
            for person_answers in group_answers.lines() {
                person_answers.chars().for_each(|c| { counter.insert(c);
                match group_counter.get(&c) {
                    Some(count) => { group_counter.insert(c, count + 1); },
                    None => { group_counter.insert(c, 1); },
                }
                });
            }
            ...
            group_answers_count += group_counter.iter().fold(0, |acc, (_, &v)| if v == voters {acc + 1} else {acc});
